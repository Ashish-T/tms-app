...
export default function AdminPanel() {
  ...
  const openBillingModal = (trip) => {
    setBillingTrip(trip);
    setBillData({
      fuel_litres: trip.fuel_litres !== null ? trip.fuel_litres : '', 
      fuel_price: trip.fuel_price !== null ? trip.fuel_price : '', 
      toll_charges: trip.toll_charges !== null ? trip.toll_charges : '', 
      other_expenses: trip.other_expenses !== null ? trip.other_expenses : '', 
      driver_cost: trip.driver_cost !== null ? trip.driver_cost : '', 
      trip_days: trip.trip_days !== null ? trip.trip_days : 1, 
      overtime_allowance: trip.overtime_allowance !== null ? trip.overtime_allowance : '', 
      advance_paid: trip.advance_paid !== null ? trip.advance_paid : '', 
      vehicle_cost_type: trip.vehicle_cost_type || 'Third Party', 
      vehicle_cost: trip.vehicle_cost !== null ? trip.vehicle_cost : '', 
      b2c_billing: trip.b2c_billing !== null ? trip.b2c_billing : '', 
  ...
  const handleGeneratePDF = (trip) => {
    ...
    const fuelCost = ((trip.fuel_litres || 0) * (trip.fuel_price || 0)).toFixed(2);
      ...
      <html>
        ...
        <body>
          ...
          <table class="financial-table">
            ...
            <tr><td>Fuel Cost (${trip.fuel_litres || 0}L @ ₹${trip.fuel_price || 0})</td><td>₹${fuelCost}</td></tr>
            <tr><td>Toll Charges</td><td>₹${trip.toll_charges || 0}</td></tr>
            <tr><td>Driver Cost (${trip.trip_days || 1} Days)</td><td>₹${trip.driver_cost || 0}</td></tr>
            <tr><td>Overtime Paid</td><td>₹${trip.overtime_allowance || 0}</td></tr>
            <tr><td>Advance Paid (Kharcha Deducted)</td><td>₹${trip.advance_paid || 0}</td></tr>
            <tr><td>Vehicle Cost (${trip.vehicle_cost_type || 'Third Party'})</td><td>₹${trip.vehicle_cost || 0}</td></tr>
            <tr><td>Other Misc. Expenses</td><td>₹${trip.other_expenses || 0}</td></tr>
            <tr class="total-row"><td>Total Running Cost</td><td>₹${trip.total_running_cost || 0}</td></tr>
          </table>
          <h3 class="section-title">Final Billing Summary</h3>
          <table class="financial-table">
            <tr><th>B2C Revenue (Billed to Client)</th><td>₹${trip.b2c_billing || 0}</td></tr>
            <tr class="profit-row"><td>Net Profit Contribution</td><td>₹${trip.profit || 0}</td></tr>
          </table>
            ...
            <table className="min-w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-300 border-b border-gray-400">
                <tr>
                  ...
                  <th className="p-4">Vehicle & Vendor</th>
                  <th className="p-4 text-indigo-700">Sourced From</th>
                  <th className="p-4">POD</th>
                  <th className="p-4">Status</th>
                  <th className="p-4 text-orange-700">Total Cost</th>
          ...
          <div className="flex min-h-full w-full items-center justify-center p-2 sm:p-4 py-8">
            <div className="bg-gray-200 p-5 md:p-6 rounded-2xl w-full max-w-5xl m-auto shadow-2xl border border-gray-400 relative">
              ...
              {!billingTrip.pod_link && (
                <div className="bg-orange-100 border border-orange-300 text-orange-800 p-3 rounded-xl mb-4 font-bold flex justify-between items-center text-sm shadow-sm">
                    ...
                    <span className="bg-white px-2 py-1 rounded text-xs text-orange-600 shadow-sm border border-orange-200">You may still Finalize the Bill.</span>
                </div>
              )}

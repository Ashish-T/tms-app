...
function DriverPanel({ userName }) {
  ...
  const handleEndTrip = async (e, trip) => {
    e.preventDefault();
    if (Number(endData.in_km) <= trip.out_km) { alert(`⚠️ Error: The 'Stop KM' reading must be higher than your Starting KM (${trip.out_km}).`); return; }
    try {
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-8">
        ...
        <form onSubmit={handleStartTrip} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          ...
          {/* Typable/Searchable Vehicle Dropdown */}
          <div className="flex flex-col space-y-1.5">
            <label className="text-sm font-semibold text-slate-700">Vehicle Number</label>
            <input 
              list="vehicles-list" name="vehicle_number" required placeholder="Type or select..." 
              value={formData.vehicle_number}
              onChange={(e) => {
                const val = e.target.value.replace(/\s+/g, '').toUpperCase(); // Removes spaces & uppercase
                setFormData(prev => ({...prev, vehicle_number: val}));
              }}
              className="bg-slate-50 border border-slate-200 text-slate-900 rounded-xl focus:ring-4 focus:ring-indigo-500/20 block w-full p-3 outline-none" 
            />
            <datalist id="vehicles-list">
              {vehicles.map((v, i) => <option key={i} value={v} />)}
            </datalist>
          </div>

          <InputField label="Reporting Time" type="time" name="reporting_time" value={formData.reporting_time} onChange={handleStartChange} />
          <InputField label="Starting KM" type="number" name="out_km" value={formData.out_km} onChange={handleStartChange} />
          <InputField label="Date (Auto)" type="date" name="date" value={formData.date} onChange={handleStartChange} />
          <InputField label="Starting Time (Auto)" type="time" name="out_time" value={formData.out_time} onChange={handleStartChange} />
          ...
          <div key={trip.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            ...
            {activeEndTrip === trip.id ? (
              <form onSubmit={(e) => handleEndTrip(e, trip)} className="flex flex-col md:flex-row gap-4 items-end bg-slate-50 p-4 rounded-xl border border-slate-200">
                <div className="flex-1 w-full"><InputField label="Stop KM" type="number" name="in_km" value={endData.in_km} onChange={handleEndChange} /></div>
                <div className="flex-1 w-full"><InputField label="Close Time (Auto)" type="time" name="in_time" value={endData.in_time} onChange={handleEndChange} /></div>
              ...
              <div>
                <button onClick={() => setActiveEndTrip(trip.id)} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-bold shadow-md hover:bg-slate-800 transition-colors">Log Arrival (Stop KM)</button>
              </div>
            )}
          </div>
        ))}
        {trips.filter(t => t.status === 'Started').length === 0 && <div className="text-slate-500 italic">No active trips right now.</div>}
      </div>
        ...
        {trips.filter(t => t.status !== 'Started').map(trip => (
          <div key={trip.id} className="bg-slate-50 p-4 rounded-xl border border-slate-200">
            <div className="flex justify-between items-center mb-2">
              <span className="font-bold text-slate-800">{trip.vehicle_number}</span>
...
function SupervisorPanel() {
  ...
  const handleReviewSubmit = async (e) => {
    e.preventDefault();
    try { await API.patch(`/trips/${reviewTrip.id}/review`, reviewData); setReviewTrip(null); fetchAllData(); } catch (err) { alert("Error saving review."); }
  };

  const handleExpenseSubmit = async (e) => {
    e.preventDefault();
    try { 
      const payload = {}; Object.keys(expData).forEach(k => payload[k] = Number(expData[k]) || 0);
      await API.patch(`/trips/${expenseTrip.id}/supervisor_expenses`, payload); 
      setExpenseTrip(null); fetchAllData(); 
    } catch (err) { alert("Error saving expenses."); }
  };

  const handleEndTrip = async (e) => {
    e.preventDefault();
    if (Number(endData.in_km) <= endingTrip.out_km) { alert(`⚠️ Error: 'Stop KM' must be higher than Starting KM (${endingTrip.out_km}).`); return; }
    try { await API.patch(`/trips/${endingTrip.id}/end`, endData); setEndingTrip(null); setEndData({ in_km: '', in_time: '' }); fetchAllData(); } catch (err) { alert("Failed to end trip."); }
  };
  ...
  const handleCreateDriver = async (e) => {
    e.preventDefault();
    try { await API.post('/users/driver', { ...driverForm, role: 'driver' }); alert("Driver Created!"); setDriverForm({ username: '', password: '', name: '', phone: '' }); fetchAllData(); } catch (err) { alert("Failed to create driver."); }
  };

  const handleAddVehicle = async (e) => {
    e.preventDefault();
    try { await API.post('/vehicles_list/', { vehicle_number: vehicleForm.replace(/\s+/g, '').toUpperCase() }); setVehicleForm(''); fetchAllData(); alert("Vehicle Added!"); } catch (err) { alert("Error adding vehicle."); }
  };

  const handleDeleteVehicle = async (id) => {
    if (window.confirm("Are you sure you want to delete this vehicle?")) {
      try { await API.delete(`/vehicles_list/${id}`); fetchAllData(); } catch(err) { alert("Failed to delete."); }
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2 mb-6 bg-slate-200/50 p-1.5 rounded-xl inline-flex">
        <button onClick={() => setActiveTab('trips')} className={`px-5 py-2.5 rounded-lg font-bold transition-all ${activeTab === 'trips' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}>Manage Trips</button>
        <button onClick={() => setActiveTab('fleet')} className={`px-5 py-2.5 rounded-lg font-bold transition-all ${activeTab === 'fleet' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}>My Fleet</button>
        <button onClick={() => setActiveTab('drivers')} className={`px-5 py-2.5 rounded-lg font-bold transition-all ${activeTab === 'drivers' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}>Add Driver</button>
                  ...
                  <td className="p-4">
                    <div className="flex gap-2">
                      {(trip.status === 'Started' || trip.status === 'Completed') && (
                         <button onClick={() => { setReviewTrip(trip); setReviewData({ vehicle_type: trip.vehicle_type || '', vehicle_mode: trip.vehicle_mode || '', body_type: trip.body_type || '', vendor_name: trip.vendor_name || '', helper_name: trip.helper_name || '' }); }} className="bg-indigo-100 text-indigo-700 px-3 py-1.5 rounded-lg font-bold hover:bg-indigo-200 text-xs shadow-sm">Review</button>
                      )}
          ...
          <div className="bg-white p-8 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleExpenseSubmit} className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <InputField label="Other Trip Expenses" type="number" value={expData.other_expenses} onChange={e => setExpData({...expData, other_expenses: e.target.value})} />
              </div>
              <div className="col-span-2 flex gap-4 mt-6">
                <button type="button" onClick={() => setExpenseTrip(null)} className="flex-1 bg-slate-100 py-3 rounded-xl font-bold text-slate-600">Cancel</button>
      ...
      {/* READ ONLY DETAILS FOR SUPERVISOR */}
      {viewingTrip && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-50">
      ...
      {activeTab === 'vehicles' && (
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg border border-slate-100">
          ...
          <form onSubmit={handleAddVehicle} className="flex flex-col gap-4 mb-6">
            <InputField label="Vehicle Number (e.g., WB123456)" value={vehicleForm} onChange={e => setVehicleForm(e.target.value)} uppercase />
            <button type="submit" className="bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700">Add</button>
          </form>
      ...
      {/* SUPERVISOR REVIEW & ARRIVAL MODALS RE-USED FROM PREVIOUS */}
      {reviewTrip && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-50">
        ...
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-8 rounded-3xl w-full max-w-sm">
            ...
            <form onSubmit={handleEndTrip} className="space-y-4">
              <InputField label="Stop KM" type="number" name="in_km" value={endData.in_km} onChange={handleEndChange} />
              <InputField label="Close Time (Auto)" type="time" name="in_time" value={endData.in_time} onChange={handleEndChange} />
              <div className="flex gap-4 mt-6">
...
function AdminPanel() {
  ...
  const handleEditDetailsSubmit = async (e) => {
    e.preventDefault();
    try { await API.patch(`/trips/${editDetailsTrip.id}/admin_edit`, editDetailsData); setEditDetailsTrip(null); fetchAllData(); } catch (err) { alert("Failed to save changes."); }
  };
  ...
  // Admin Creations & Deletions
  const handleCreateSupervisor = async (e) => {
    e.preventDefault();
    try { await API.post('/users/supervisor', { ...supForm, role: 'supervisor' }); alert("Created!"); setSupForm({ username: '', password: '', name: '', phone: '' }); fetchAllData(); } catch (err) { alert("Error."); }
  };
  const handleDeleteUser = async (id) => {
    if (window.confirm("Delete this user permanently?")) {
      try { await API.delete(`/users/${id}`); fetchAllData(); } catch(err) { alert("Failed."); }
    }
  };

  const handleAddVendor = async (e) => {
    e.preventDefault();
    try { await API.post('/vendors_list/', { name: vendorName }); setVendorName(''); fetchAllData(); } catch (err) { alert("Error."); }
  };
  const handleDeleteVendor = async (id) => {
    if (window.confirm("Delete vendor?")) { try { await API.delete(`/vendors_list/${id}`); fetchAllData(); } catch(err) { alert("Failed."); } }
  };

  const handleAddVehicle = async (e) => {
    e.preventDefault();
    try { await API.post('/vehicles_list/', { vehicle_number: vehicleForm.replace(/\s+/g, '').toUpperCase() }); setVehicleForm(''); fetchAllData(); } catch (err) { alert("Error."); }
  };
  const handleDeleteVehicle = async (id) => {
    if (window.confirm("Delete vehicle?")) { try { await API.delete(`/vehicles_list/${id}`); fetchAllData(); } catch(err) { alert("Failed."); } }
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2 mb-6 bg-slate-200/50 p-1.5 rounded-xl inline-flex">
        ...
        <button onClick={() => setActiveTab('supervisors')} className={`px-5 py-2.5 rounded-lg font-bold transition-all ${activeTab === 'supervisors' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}>Supervisors</button>
        <button onClick={() => setActiveTab('vendors')} className={`px-5 py-2.5 rounded-lg font-bold transition-all ${activeTab === 'vendors' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}>Vendors</button>
        <button onClick={() => setActiveTab('vehicles')} className={`px-5 py-2.5 rounded-lg font-bold transition-all ${activeTab === 'vehicles' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}>Vehicles</button>
      </div>
      ...
      {activeTab === 'trips' && (
        <div className="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-x-auto">
          <table className="min-w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 border-b">
              <tr><th className="p-4">ID</th><th className="p-4">Date</th><th className="p-4">Personnel</th><th className="p-4">Vehicle & Vendor</th><th className="p-4">Status</th><th className="p-4">Profit</th><th className="p-4">Action</th></tr>
            </thead>
            <tbody>
              {trips.map(trip => (
                <tr key={trip.id} className="border-b">
                  <td className="p-4 font-bold">#{trip.id}</td>
                  <td className="p-4 text-slate-600 font-semibold">{trip.date}</td>
                  <td className="p-4">
      ...
      {/* TABS WITH DELETION FUNCTIONALITY */}
      {activeTab === 'supervisors' && (
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg border border-slate-100">
          ...
          <form onSubmit={handleCreateSupervisor} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold">Add Supervisor</button>
          </form>
          
          <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider border-b pb-2 mb-4 mt-8">Active Supervisors</h4>
          <div className="grid grid-cols-1 gap-2">
            {users.filter(u => u.role === 'supervisor').map(sup => (
              <div key={sup.id} className="bg-slate-50 p-3 rounded-lg border text-sm font-semibold flex items-center justify-between">
                <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-indigo-500"></span>{sup.name} ({sup.username})</div>
                <button onClick={() => handleDeleteUser(sup.id)} className="text-rose-500 hover:text-rose-700 px-2 py-1 rounded bg-rose-100 text-xs">Delete</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {activeTab === 'vendors' && (
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg border border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-2">
            {vendors.map(v => (
              <div key={v.id} className="bg-slate-50 p-2 rounded-lg border text-sm font-semibold flex items-center justify-between">
                <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500"></span>{v.name}</div>
                <button onClick={() => handleDeleteVendor(v.id)} className="text-rose-500 hover:text-rose-700">✕</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {activeTab === 'vehicles' && (
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg border border-slate-100">
          <h3 className="text-xl font-bold mb-4">Register New Vehicle</h3>
          <form onSubmit={handleAddVehicle} className="flex flex-col gap-4 mb-6">
            <InputField label="Vehicle Number (No Spaces)" value={vehicleForm} onChange={e => setVehicleForm(e.target.value)} uppercase />
          ...
          <div className="grid grid-cols-2 gap-2">
            {vehicles.map(v => (
              <div key={v.id} className="bg-slate-50 p-2 rounded-lg border text-sm font-bold text-slate-700 flex items-center justify-between">
                <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500"></span>{v.vehicle_number}</div>
                <button onClick={() => handleDeleteVehicle(v.id)} className="text-rose-500 hover:text-rose-700">✕</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ADMIN READ ONLY LOG, EDIT MODALS & FINANCE MODAL RE-USED */}
      {viewingTrip && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-8 rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold">Trip Log (#{viewingTrip.id})</h3>
              <button onClick={() => setViewingTrip(null)} className="text-slate-400 font-bold bg-slate-100 p-2 rounded-full">✕</button>
            </div>
      ...
      {/* FLEET OVERVIEW TAB REMAINS THE SAME... */}
      {activeTab === 'fleet' && (

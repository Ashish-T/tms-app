      ...
      <div className="min-h-screen font-sans text-gray-900 bg-[url('/futuristic-hud-interface-background-with-blue-gradient-technology-circles-vector_2.jpg')] bg-cover bg-center bg-fixed">
        ...
        <nav className="bg-sky-600/95 backdrop-blur-sm px-6 py-4 sticky top-0 z-40 shadow-xl flex justify-between items-center text-gray-200 border-b border-sky-500/50">
          ...
          <div className="flex items-center gap-4">
            <div className="text-sm hidden sm:block drop-shadow-sm">Logged in as <span className="font-bold text-gray-100">{name}</span></div>
            <button onClick={() => setShowSettings(true)} className="bg-sky-700/80 hover:bg-sky-800 px-4 py-2 rounded-lg font-bold transition-colors text-sm shadow-inner border border-sky-500/30">Settings</button>
            <button onClick={handleLogout} className="bg-orange-500/90 hover:bg-orange-600 px-4 py-2 rounded-lg font-bold transition-colors text-sm text-white shadow-md border border-orange-400/50">Logout</button>
          </div>
        </nav>
        
        {showSettings && (
            <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                <div className="bg-gray-200 p-8 rounded-3xl w-full max-w-sm shadow-2xl border border-gray-300">
                    ...
                    <form onSubmit={handlePasswordChange} className="space-y-4">
                        ...
                        <div className="flex gap-4 mt-6">
                            <button type="button" onClick={() => setShowSettings(false)} className="flex-1 bg-gray-400 py-3 rounded-xl font-bold text-gray-800 hover:bg-gray-500 shadow-sm">Cancel</button>
                            <button type="submit" className="flex-1 bg-emerald-600 text-gray-200 py-3 rounded-xl font-bold hover:bg-emerald-700 shadow-md">Update Securely</button>
                        </div>
                    </form>
                </div>
            </div>
